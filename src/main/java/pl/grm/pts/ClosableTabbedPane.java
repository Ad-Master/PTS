		...
		public void paint(Graphics g) {
			...
			for (int j = 0; j < tabCount; j++) {
				if (this.tabbedPane.getComponent(j).isShowing()) {
					int x = (this.tabbedPane.getBoundsAt(j).x + this.tabbedPane.getBoundsAt(j).width)
							- this.width - 5;
		...
		private boolean mouseOverTab(int x, int y) {
			...
			for (int j = 0; j < tabCount; j++) {
				if (this.tabbedPane.getBoundsAt(j).contains(this.meX, this.meY)) {
					this.selectedTab = j;
					this.closeX = (this.tabbedPane.getBoundsAt(j).x + this.tabbedPane
							.getBoundsAt(j).width) - this.width - 5;
